 ...
 * and the Contact-page business details — types, seed defaults and reconcile.
 *
 * SERVER-SAFE (no "use client", no browser APIs) so both the client hook
 * (`sitePages.ts`) and the API route (`/api/content/pages`) share it. The
 * content is persisted in the database (the `settings` singleton table) via
 * that route; this module owns only the shape + defaults.
 */
...
export function reconcile(stored: Partial<SiteContent> | null): SiteContent {
