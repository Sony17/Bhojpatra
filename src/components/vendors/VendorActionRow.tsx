...
export default function VendorActionRow({
  ...
  return (
    <div className={`grid grid-cols-4 gap-1 sm:gap-2.5 ${className}`}>
      {/* 1. Book Now / Quantity Counter */}
      {typeof quantity === "number" && onQuantityChange ? (
        quantity > 0 ? (
          <div className="flex min-h-[2.75rem] items-stretch overflow-hidden rounded-full border border-[#E5C185] bg-white shadow-xs">
            <button
              type="button"
              onClick={() => onQuantityChange(Math.max(0, quantity - 1))}
              className="focus-ring flex flex-1 items-center justify-center text-lg font-bold text-maroon transition hover:bg-cream/40 active:scale-95"
              aria-label="Decrease quantity"
            >
              −
            </button>
            <span className="flex min-w-[1.75rem] sm:min-w-[2.25rem] items-center justify-center border-x border-[#E5C185]/60 px-1 text-center text-xs sm:text-sm font-bold tabular-nums text-ink">
              {quantity}
            </span>
            <button
              type="button"
